...
class Scraper(CommonWords):
    ...
    def __init__(self, username: str, password: str, query: str, n_pages: int, quantity=50) -> None:
        """
    ...
    def search_list(self) -> list:
        ...
        for page in range(1, self.n_pages):
            ...
            time.sleep(2)
            search = self.driver.find_elements(By.XPATH, '//div[@class="yuRUbf"]//a[@href]')
            if len(search) <= 0:
                search = self.driver.find_elements(By.XPATH, '//div[@class="kCrYT"]//a[@href]')
            for h in search:
    ...
    def loading_all_elements(self) -> None:
        ...
        containers = self.driver.find_elements(By.XPATH, "//li-icon[@class='pv-profile-section__toggle-detail-icon']")
